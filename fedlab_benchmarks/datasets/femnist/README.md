...
- Run ```./preprocess.sh``` with a choice of the following tags:
    ...
    - ```--iu``` := number of users, if iid sampling, ignored in niid cases; expressed as a fraction of the total number of users; default is 0.01
    - ```--sf``` := fraction of data to sample, written as a decimal; default is 0.1
    - ```-k``` := minimum number of samples per user, discard any users with samples less than such
